/*
  ...
  - You are about to drop the column `address` on the `ong_profiles` table. All the data in the column will be lost.
...
-- AlterTable
ALTER TABLE `ong_profiles` DROP COLUMN `address`,
...
CREATE TABLE `addresses` (
    ...
    UNIQUE INDEX `addresses_donorId_key`(`donorId`),
    UNIQUE INDEX `addresses_ongId_key`(`ongId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `addresses` ADD CONSTRAINT `addresses_donorId_fkey` FOREIGN KEY (`donorId`) REFERENCES `donors`(`userId`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `addresses` ADD CONSTRAINT `addresses_ongId_fkey` FOREIGN KEY (`ongId`) REFERENCES `ongs`(`userId`) ON DELETE CASCADE ON UPDATE CASCADE;
